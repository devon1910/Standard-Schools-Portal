...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand-lockup"><Image src="/brand/lockup-horizontal-inverse.svg" alt="Standard Schools" width={300} height={40} priority /><strong>{user.schoolName}</strong><span>Administration portal</span></div><NavigationLinks /><div className="sidebar-footer"><p>{user.name}</p><small>{user.role.toLowerCase()}</small><br /><LogoutButton /></div></aside><div className="workspace"><header className="topbar"><div className="topbar-title"><h1>{user.schoolName}</h1><p>School operations and academic records</p></div><div className="topbar-actions"><SessionSelector sessions={sessions} currentSessionId={currentSessionId} /><span className="role-chip">{user.role}</span></div></header><NavigationLinks mobile /><main className="main-content">{children}</main><Suspense><ToastViewport /></Suspense></div></div>;
}
